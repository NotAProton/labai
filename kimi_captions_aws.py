...
SYSTEM_PROMPT = """\
You are a cloud computing lab report assistant. Your ONLY job is to analyze screenshots 
of various cloud tools like AWS Dashboard and produce structured JSON that drives
an automated LaTeX report-generation pipeline.

You must return a JSON object with the following structure:
{
    "likely_tools": [list of tools likely shown in the screenshot, e.g. "AWS EC2", "S3", "CloudWatch", "Lambda"],
    "observations": [list of detailed observations about the screenshot, e.g. "There is a table showing instance types and statuses", "The title bar indicates..."],
...
def _call_converse(client, system_prompt: str, content: list[dict],
    ...
    now = time.time()
    if now - last_call_time < REQUEST_DELAY:
        print(f"Throttling request to respect {REQUEST_DELAY}s delay between calls...", file=sys.stderr)
        time.sleep(REQUEST_DELAY - (now - last_call_time))
...
"""
    ...
    for attempt in range(3):
        ...
        required = {"likely_tools", "observations", "summary"}
